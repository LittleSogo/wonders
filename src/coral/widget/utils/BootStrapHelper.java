/**
 * @Project ZMPlatForm
 * @Title BootStrapHelper.java
 * @Package coral.widget.utils
 * @Author Ken xuchao@wondesgroup.com
 * @CreateDate Mar 10, 2016 11:26:59 AM
 * @Copyright 2016
 * @version V1.0 
 */
package coral.widget.utils;
...
public class BootStrapHelper {
	...
	public static class Page implements WidgetPage {
		 ...
		 *            SQL的额外条件
		 */
		protected Page(String pageNo, String pageSize, String sort, String dir, String suffix) {
			try {
			...
			if (!StringUtils.trimToEmpty(sort).isEmpty() && !StringUtils.trimToEmpty(dir).isEmpty()) {
				if (sort.contains(",") && dir.contains(",")) {
					...
					if (sorts.length == dirs.length) {
						for (int i = 0; i < sorts.length; i++) {
							...
							if (SqlUtils.checkSqlSymbol(subSort) && SqlUtils.checkSqlSymbol(subDir)) {
								if (sb.length() > 0)
									sb.append(", ");
					...
					if (SqlUtils.checkSqlSymbol(sort) && SqlUtils.checkSqlSymbol(dir))
	...
	public static Page getPage(HttpServletRequest req) {
		return new Page(req.getParameter(PAGE_NO), req.getParameter(PAGE_SIZE), req.getParameter(SORT_PARAM), req.getParameter(DIR_PARAM), req.getParameter(SUFFIX_PARAM));
	}
	...
	public static Page getPage(RequestWrapper wrapper) {
		return new Page(wrapper.getParameter(PAGE_NO), wrapper.getParameter(PAGE_SIZE), wrapper.getParameter(SORT_PARAM), wrapper.getParameter(DIR_PARAM), wrapper.getParameter(SUFFIX_PARAM));
	}
	 ...
	 * @throws IOException
	 */
	public static void writeResponse(HttpServletResponse res, String result) throws IOException {
		writeResponse(res, result, SystemConstant.DEFAULT_CHARSET);
	}

	public static void writeFormResponse(HttpServletResponse res, String result, String charset) throws IOException {
		result = StringEscapeUtils.escapeHtml(result);
		res.setContentType("text/html;charset=" + SystemConstant.DEFAULT_CHARSET);
	 ...
	 * @throws IOException
	 */
	public static void writeFormResponse(HttpServletResponse res, String result) throws IOException {
	 ...
	 * @throws IOException
	 */
	public static void writeResponse(HttpServletResponse res, String result, String charset) throws IOException {
	 ...
	 * BootStrap查询参数：排序方向
	 */
	public static final String DIR_PARAM = "sortOrder";
	 ...
	 * BootStrap查询参数：排序字段
	 */
	public static final String SORT_PARAM = "sortName";

	/**
	 * BootStrap查询参数：SQL额外条件
	 */
